//! 笔记用例服务。
//!
//! 与 [`crate::application::task_service::TaskService`] 相同的用例编排模式，实现
//! [`NoteUseCase`] 北向端口。

use crate::application::error::ServiceError;
use crate::application::ports::NoteUseCase;
use crate::application::{CreateNoteDto, UpdateNoteDto};
use crate::domain::{DomainError, Note, NoteRepository};
use std::sync::Arc;

/// 笔记用例服务，持有一个 `NoteRepository` 南向端口。
#[derive(Clone)]
...
impl NoteUseCase for NoteService {
    /// 创建笔记：校验内容 → 生成实体 → 持久化。
