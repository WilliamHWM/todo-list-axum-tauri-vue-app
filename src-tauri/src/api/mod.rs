//! Axum HTTP API layer.
//!
//! Defines the shared application state, the router, error/response types,
//! extractors and feature-scoped handlers.
...
///
/// Extend this struct as the application grows (e.g. caches, clients).
#[derive(Clone)]
pub struct AppState {
    pub db: SqlitePool,
...
mod tests {
    ...
    async fn test_state() -> AppState {
        ...
        let pool = crate::db::init_pool(&config).await.expect("init_pool failed");
    ...
    async fn create_task_returns_task_with_utc_timestamp() {
        let app = create_router(test_state().await);
        let (status, body) = json_response(app, post_json("/api/tasks", r#"{"title":"写测试"}"#)).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body["code"], 0);
    ...
    async fn create_task_rejects_blank_title() {
        let app = create_router(test_state().await);
        let (status, body) = json_response(app, post_json("/api/tasks", r#"{"title":"   "}"#)).await;
    ...
    async fn list_tasks_paginates() {
        let app = create_router(test_state().await);
        for i in 0..3 {
            json_response(app.clone(), post_json("/api/tasks", &format!(r#"{{"title":"任务{i}"}}"#))).await;
        }
